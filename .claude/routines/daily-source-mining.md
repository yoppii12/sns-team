...
- Slack 完了通知先チャンネル：channel_id `C0BA5L04TE0`（https://laplust.slack.com/archives/C0BA5L04TE0）
...
- Notion「下書き置き場」データベース：data_source_id `collection://1f242440-084c-4786-9700-f4110d5f450b`（URL: https://www.notion.so/laplust/SNS-Team-39dc08e5f34c801f8099d2715196b572 配下）
  - 「作成日時」プロパティ（Created time型）を追加済み。ネタ候補がいつ登録されたか一覧で確認できる
...
### ステップ6: Slackへの完了通知
ステップ1〜5がすべて完了したら、`mcp__Slack__slack_send_message` でSlackチャンネル（channel_id: `C0BA5L04TE0`）に完了報告を送る。
- ネタ候補を登録した場合：登録した件数と、各候補のタイトル・媒体・目的タグ・機密チェック状況（Notionページへのリンク）を簡潔にまとめる
- 候補が無かった場合：「前日分の新しい議事録が見つからなかったため、本日は登録なし」の旨を送る
...
- 2026-07-16：yoshikaさんがclaude.ai Routines画面から作成・実行開始（Slack・Notion・Google Calendarコネクタが接続済み。**Googleドライブコネクタは未接続のため要確認**）。Notion「下書き置き場」に「作成日時」プロパティを追加。Routine完了時にSlackチャンネル（`C0BA5L04TE0`）へ完了報告を送るステップ6を追加
