...
- **実行方式**：毎回新規セッションを作成して実行（`create_new_session_on_fire: true`）。日々の会話履歴は残らない
...
- X：日本語120〜140字程度
- note：1,500〜3,000字（小粒なネタは無理に埋めない）
- YouTube：台本＋構成メモ
...
- 機密性が高いと判断した内容は、具体的詳細を本文に含めない方向に必ず倒す
- 顧客名・契約情報・未公開の数値実績・開発中の未発表機能・社内の人間関係に関するネガティブな発言は本文に含めない。含める必要がありそうな場合は「【要相談】」として具体的に何を判断してほしいか記載する
- 社員個人の発言は実名表記でよい
